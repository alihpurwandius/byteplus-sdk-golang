...
func (c *Live) ListStorageSpace(arg *ListStorageSpaceBody) (*ListStorageSpaceRes, error) {
	...
	data, _, err := c.Client.Json("ListStorageSpace", url.Values{}, string(body))
	if err != nil {
		return nil, err
	}

	result := new(ListStorageSpaceRes)
...
func (c *Live) ListStorageSpaceDetail(arg *ListStorageSpaceDetailBody) (*ListStorageSpaceDetailRes, error) {
	...
	data, _, err := c.Client.Json("ListStorageSpaceDetail", url.Values{}, string(body))
	if err != nil {
		return nil, err
	}

	result := new(ListStorageSpaceDetailRes)
	err = unmarshalResultInto(data, result)
	if err != nil {
		return nil, err
	}
